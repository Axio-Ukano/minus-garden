...
pub fn save_session(state: tauri::State<'_, DbState>, session: Session) -> Result<(), String> {
    ...
    conn.execute(
        "INSERT OR REPLACE INTO sessions
            (id, start_time, end_time, duration_minutes, subject, completed, hearts_earned)
...
pub fn get_sessions(state: tauri::State<'_, DbState>) -> Result<Vec<Session>, String> {
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    let mut stmt = conn
        .prepare("SELECT id, start_time, end_time, duration_minutes, subject, completed, hearts_earned FROM sessions ORDER BY start_time DESC")
        .map_err(|e| e.to_string())?;
