...
| In-app credits          | Settings → **ABOUT** tab (`sections/AboutSection.tsx`, i18n `settings.about`) | version is **dynamic** via `__APP_VERSION__` (injected from `package.json` in `vite.config.ts`/`vitest.config.ts`) |

### Where new signatures go

- **New first-party `.ts/.tsx/.rs` file** → must start with the 3-line header. This is **enforced by CI**: `pnpm check:headers` (script `scripts/check-license-headers.mjs`, also part of `pnpm validate` and a step in `validate.yml`) fails the build listing any source file without the SPDX line. Exemptions live in that script's `EXCLUDE` list: generated/vendor (`node_modules/`, `src-tauri/target/`, `dist/`) and Vite-generated `vite-env.d.ts` (header maintained by hand). To add a new exemption, edit that list and this section together.
...
- **Year range.** The header/notice end year (`2026`) is a manual constant in `LICENSE`, the README, every source header, and `sections/AboutSection.tsx`. When the calendar year of a _substantive_ change rolls over (e.g. first real change in 2027), bump `2024–<new year>` in all four surfaces in one `chore` PR. Note `check:headers` only checks the SPDX id, not the year — a stale year won't fail CI. Consider scripting the bump (single source → codemod) before the next one if it becomes tedious.
...
- **Enforcement.** `pnpm check:headers` gates the SPDX header in CI (`validate.yml`) and locally via `pnpm validate`. It validates presence of the SPDX id only — not the copyright year, the author string, or the LICENSE/README/in-app surfaces, which stay manual. If you ever need stricter checks (e.g. fail on a wrong year or a drifted author line), extend `scripts/check-license-headers.mjs`.
